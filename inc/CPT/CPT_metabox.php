...
add_action( 'add_meta_boxes', 'cruiser_meta_box_callback' );
add_action( 'save_post' , 'cruiser_save_topic_writer');

function cruiser_meta_box_callback( $post ){
    add_meta_box('cruiser_author_meta_box', 'Select Writer', 'cruiser_author_meta_box_callback', 'topics', 'side' , 'high');
    add_meta_box( 'cruiser_topic_pick_box', 'Pick Topic', 'cruiser_topic_pick_box_callback', 'topics', 'side', 'high' );
...
function cruiser_topic_pick_box_callback( $post ) {
    ...
    ?>
    <input type="checkbox" name="topic_checkbox_value" id="topic_checkbox_value-<?php echo $post->ID; ?>" value="1" <?php checked( $topic_checkbox_value, 1 ); ?> style=""/>
    <?php
